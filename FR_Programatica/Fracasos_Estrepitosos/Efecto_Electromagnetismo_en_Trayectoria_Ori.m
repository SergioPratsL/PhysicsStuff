...
% Choque frontal, viniendo desde muy lejos
pos_1_ini = [-500000000, 0, 0];
pos_2_ini = [500000000, 0, 0];
...
% v^2(1+p^2) = p^2 / m^2   
    mom_1_cuadr = norm(mom_1(iter,:))^2;
    mom_2_cuadr = norm(mom_2(iter,:))^2;
    v_1_abs = sqrt( mom_1_cuadr / (1 + mom_1_cuadr) ) / m1;
    v_2_abs = sqrt( mom_2_cuadr / (1 + mom_2_cuadr) ) / m2;
    
    vel_1(iter,:) = v_1_abs * mom_1(iter,:) / norm(mom_1(iter,:));
    vel_2(iter,:) = v_2_abs * mom_2(iter,:) / norm(mom_2(iter,:)); 
    
    v_abs_1 = norm(vel_1(iter,:));
    v_abs_2 = norm(vel_2(iter,:));
