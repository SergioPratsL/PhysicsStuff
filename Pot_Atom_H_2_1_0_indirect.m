...
fileID = fopen('C:\Datos\Pot_Atm_H_2_1_0_indir.txt','w');
...
%    ang_actual = - pi / 2 + iter_ang_actual * ang_unit;
    ang_actual = iter_ang_actual * ang_unit;
    seno(iter_ang_actual) = sin(ang_actual);
    seno_cuad(iter_ang_actual) = sin(ang_actual)^2;
...
% Loop para evaluar las contribuciones de cada superficie esferica  
        while iter_rad_int <= num_iter_r      % Loop 3: contribuciones de cada radio, "int" significa interno
            iter_ang_int = 1;
%            ang_actual = - pi / 2 + ang_unit * iter_ang_actual;
            ang_actual = ang_unit * iter_ang_actual;
...
% El valor de la carga es a nivel de superficie esferica
            ...
            dens_carg = e * cte * exp(- r_int / a0) * (r_int / a0)^2;
...
% Superficie (mas bien 'grosor') que corresponde a cada 'aro'    
                if iter_ang_int == 1
                    AngSolid = 2 * pi * (1 - coseno(1));
                else
                    AngSolid = 2 * pi * (coseno(iter_ang_int - 1) - coseno(iter_ang_int));
                end

% Da 2 al final de cada loop :)                
%                superf_acum = superf_acum + superf;             
                
% Carga de la linea: al ser el nivel de energia 2_1_0 SI es necesario obtenerla                
                cos_fase_cuad = seno_cuad(iter_ang_actual);     % r · z / (|r|*|z|) = seno(and)     % OK
                seno_fase_cuad = cos_cuad(iter_ang_actual);                                         % OK
              
% Ver formula del 28.12.2016 para entender este embrollo :).                
%                factor_carga = 1 -  cos_fase_cuad * seno_cuad(iter_ang_int) - (1/2) * seno_fase_cuad * cos_cuad(iter_ang_int);
                factor_carga = cos_fase_cuad * cos_cuad(iter_ang_int) + (1/2) * seno_fase_cuad * seno_cuad(iter_ang_int);

                pot_integrado_radio = pot_integrado_radio +  factor_carga * AngSolid / dist;
                
                Vol_tot = Vol_tot + AngSolid;
                Fact_tot = Fact_tot + AngSolid * factor_carga;
