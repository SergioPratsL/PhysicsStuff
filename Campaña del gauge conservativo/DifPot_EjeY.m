...
function [Pot_A, Pot_B, F_rad, Dif] = DifPot_EjeY( Rx, Ry,  v)
...
% Obtener las derivadas espaciales desde el SRI de B:
dPot_A_dx = DerivadasPotencialYinn( Rr_B_t(2), Rr_B_t(3), -v, 'x');

% Ojo, revisar pues no se si se ha probado nunca!!
dPot_A_dy = DerivadasPotencialYinn( Rr_B_t(2), Rr_B_t(3), -v, 'y');

% La rotacion efectuada es dv / (v*Sigma), espero que la precesion de
% Thomas no lo joda todo...

% Ojo! El sigmo menos se debe a que en este caso el emisor el B y quien
% acelera es A. Pero como v tambien es -v, se compensa, no hay que poner
% signo menos!
dRot_1 = 1 / (v*Sigma);
% dRot_1 = 1 / (v);       % Un paso en falso! (es que esta vez es el fin)

% Variacion debido a como la primera rotacion altera Rx y Ry
dV_dRot_A = - dRot_1 * Rr_B_t(3) * dPot_A_dx(1) + dRot_1 * Rr_B_t(2) * dPot_A_dy(1);

dAx_dRot_A = - dRot_1 * Rr_B_t(3) * dPot_A_dx(2) + dRot_1 * Rr_B_t(2) * dPot_A_dy(2);

dAy_dRot_A = - dRot_1 * Rr_B_t(3) * dPot_A_dx(3) + dRot_1 * Rr_B_t(2) * dPot_A_dy(3);

dAz_dRot_A = - dRot_1 * Rr_B_t(3) * dPot_A_dx(4) + dRot_1 * Rr_B_t(2) * dPot_A_dy(4);
...
dAx_desrot_A = dRot_1 * A_A_desde_B(2);

dAy_desrot_A = - dRot_1 * A_A_desde_B(1);
...
% Primero  se hace un miniboost que modifica Ry en  - dv* R, la
...
dPot_B_mini_boost = - dist_f * dPot_B_dy;

% Al no ser en direccion X la velocidad vista desde A', hay que hacer la
% mini rotacion (todo es mini de hecho, excepto los boosts de un sistema al
% otro. En este caso la rotacion no llevara el factor 1/Sigma y el signo es
% el contrario

 dRot_2 = 1 / v;
% dRot_2 = 1 / (v*Sigma);     % Blasfemia

% Otra blasfemia: contraccion de longitud en X... Las blasfemias no han
% resuelto nada.

dV_dRot_B = - dRot_2 * Rf(2) * dPot_B_dx(1) + dRot_2 * Rf(1) * dPot_B_dy(1);

dAx_dRot_B = - dRot_2 * Rf(2) * dPot_B_dx(2) + dRot_2 * Rf(1) * dPot_B_dy(2);

dAy_dRot_B = - dRot_2 * Rf(2) * dPot_B_dx(3) + dRot_2 * Rf(1) * dPot_B_dy(3);   % Parece OK

dAz_dRot_B = - dRot_2 * Rf(2) * dPot_B_dx(4) + dRot_2 * Rf(1) * dPot_B_dy(4);
...
dAx_desrot_B = dRot_2 * A_B_desde_A(2);

dAy_desrot_B = - dRot_2 * A_B_desde_A(1);
...
dV_boost_inv_B = A_B_desde_A(2);

dAx_boost_inv_B = 0;

dAy_boost_inv_B = V_B_desde_A;
...
% Z1 = dRot_2 * Rf(1) - dist_f + dRot_1 * Rr_B_t(2)

% Z1_B = + dRot_1 * Rr_B_t(2) * dPot_A_dy(3) + dRot_2 * Rf(1) * dPot_B_dy(3) + dPot_B_mini_boost(3)

% Z2 = - dRot_2 * Rf(2) - dRot_1 * Rr_B_t(3) 