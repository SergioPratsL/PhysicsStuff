function Deriv2 = SegundDerivPotYinnYX( Rx, Ry, v, dv_e, dv_r, despl )
...
% V. Efecto de despl sobre la distancia del Miniboost:
    V_pot = V_pot - dv_e * despl * (Rx/R) * d1_V(2);
    
% V. Efecto de despl sobre (dV/dv)
    dif_1_V = - dv_e * despl * (1/v) * ( -Ry*ddV(1,1) + Rx*ddV(1,2) ) / Sigma;
    dif_2_V = 0;
    V_pot = V_pot + dif_1_V + dif_2_V;
    
% V. Efecto de despl sobre el boost inverso
    V_pot = V_pot + dv_e * despl * d1_Ay(1);
...
% Ax. Efecto de despl sobre la distancia del Miniboost:        
    Ax = Ax - dv_e * despl * (Rx/R) * d1_Ax(2);
    
% Ax. Efecto de despl sobre (dV/dv).
    dif_1_Ax = - dv_e * despl * (1/v) * ( -Ry*ddAx(1,1) + Rx*ddAx(1,2) ) / Sigma;
    dif_2_Ax = - dv_e * despl * (1/v) * d1_Ay(1) / Sigma;
    Ax = Ax + dif_1_Ax + dif_2_Ax;    
    
% Ax. Efecto de despl sobre el boost inverso_ no aplica
...
% Ay. Efecto de despl sobre la distancia del Miniboost:      
    Ay = Ay - dv_e * despl * (Rx/R) * d1_Ay(2);

% Ay. Efecto de despl sobre (dV/dv).    
    dif_1_Ay = - dv_e * despl * (1/v) * ( -Ry*ddAy(1,1) + Rx*ddAy(1,2) ) / Sigma;
    dif_2_Ay = dv_e * despl * (1/v) * d1_Ax(1) / Sigma;
    Ay = Ay + dif_1_Ay + dif_2_Ay;    

% Ay. Efecto de despl sobre el boost inverso: no hay (no afecta a Ay)  
    Ay = Ay + dv_e * despl * d1_V(1);    
...
% V. Efecto de dv_r sobre el Miniboost
    V_pot = V_pot + dv_e * dv_r * (R/v) * d1_V(2);

% V. Efecto de dv_r sobre (dV/dv), no aplica    

% V. Efecto de modificar el Sigma que afecta a (dV/dv)
    dif_1_V = dv_e * dv_r * Sigma * ( -Ry*d1_V(1) + Rx*d1_V(2) );
    dif_2_V = 0;
    V_pot = V_pot + dif_1_V + dif_2_V;

% V. Efecto de dv_r sobre el boost inverso 
    V_pot = V_pot - dv_e * dv_r * (1/v) * A_yinn(2);
    
    
% Ax. Efecto de dv_r sobre el Miniboost
    Ax = Ax + dv_e * dv_r * (R/v) * d1_Ax(2);

% Ax. Efecto de dv_r sobre (dAx/dv).
    dif_1_Ax = dv_e * despl * (1/v^2) * ( -Ry*d1_Ax(1) + Rx*d1_Ax(2) ) / Sigma;
    dif_2_Ax = dv_e * despl * (1/v^2) * A_yinn(2) / Sigma;
    Ax = Ax + dif_1_Ax + dif_2_Ax;    

% Ax. Efecto de modificar el Sigma que afecta a (dAx/dv)
    dif_1_Ax = dv_e * dv_r * Sigma * ( -Ry*d1_Ax(1) + Rx*d1_Ax(2) );
    dif_2_Ax = dv_e * dv_r * Sigma * A_yinn(2);
    Ax = Ax + dif_1_Ax + dif_2_Ax;  
...
% Ay. Efecto de dv_r sobre el Miniboost
    Ay = Ay + dv_e * dv_r * (R/v) * d1_Ay(2);

% Ay. Efecto de dv_r sobre (dAy/dv)
    dif_1_Ay = dv_e * despl * (1/v^2) * ( -Ry*d1_Ay(1) + Rx*d1_Ay(2) ) / Sigma;
    dif_2_Ay = - dv_e * despl * (1/v^2) * A_yinn(1) / Sigma;
    Ay = Ay + dif_1_Ay + dif_2_Ay;    

% Ay. Efecto de modificar el Sigma que afecta a (dAy/dv): no hay    
    dif_1_Ay = dv_e * dv_r * Sigma * ( -Ry*d1_Ay(1) + Rx*d1_Ay(2) );
    dif_2_Ay = - dv_e * dv_r * Sigma * A_yinn(1);
    Ay = Ay + dif_1_Ay + dif_2_Ay;  

% Ay. Efecto de dv_r sobre el boost inverso: no hay (dV/dv = 0)
