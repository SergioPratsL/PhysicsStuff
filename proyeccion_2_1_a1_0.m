...
cte_2 = 1/(32*pi*a0^3);
cte_onda_2 = sqrt( cte_2 );     % La constante anterior 'cte' es la constante de la onda al cuadrado

cte_onda_1 = 1/(sqrt(pi)) / a0^(3/2);

cte = cte_onda_1 * cte_onda_2;


Permitividad_vacio = 8.85419 *10^-12; 
cte_poten = e / (4*pi*Permitividad_vacio);



% La variable nivel controla si evaluamos la autoenergia para el nivel del
% atomo de hidrogeno '2_0_0' o para el '2_1_0'.
nivel = '2_1_0';                    % Y no debe cambiar!
...
%r_unit = a0 / 10;
r_unit = a0 * 0.2;        
r_unit = a0 * 0.01;       % Test para comprobar una hipotesis!!  ... Estaba mal dicha hipotesis...
%num_iter_V = 500;
num_iter_V = 300;
...
onda_lv_2_acumulada = 0;
onda_lv_1_acumulada = 0;
valor_acumulado = 0;
check_ortogonal = 0;
...
% Guarradilla: para sacar la interpolacion de los angulos me basare en los cosenos y no en los angulos.
            ...
            if dist_ang == 0    % Excepcion
                ...
                V2_r = (seno(num_sen_post) - colatitud) * V(num_sen_post, num_r_post) + (colatitud - seno(num_sen_ant)) * V(num_sen_ant, num_r_post) ;
                V2_r = V2_r / dist_ang;
                
            end
            
% Por fin obtenemos el potencial interpolado!
            r2 = r_unit_V * num_r_post;
            r1 = r_unit_V * num_r_ant;
            ...
            elseif r == r1
                ...
                V_obtenido = V_obtenido / r_unit_V;         
            end
                        
% Sumamos el potencial del nucleo!!!
%            V_obtenido = V_obtenido - cte_poten / r;
%            V_obtenido = - cte_poten / r;           % Da una frecuencia de menos de Hz!!

%             if V_obtenido > 1000 * (cte_poten / r)
%                 problema = [cont_x, cont_y, cont_z]
%             end

            if nivel == '2_1_0'
                cos_ang = colatitud;            % Por usar el rango de -pi/2 a pi/2 (mas facil que cambiar todo lo demas) :).
                f_onda_lv_2 = (r / a0) * exp_onda * cos_ang;                               
            end
        
            f_onda_lv_1 = exp_onda_lv_1;            
            
            valor_acumulado = valor_acumulado + f_onda_lv_2 * f_onda_lv_1 * V_obtenido;
            onda_lv_2_acumulada = onda_lv_2_acumulada + f_onda_lv_2^2;
            onda_lv_1_acumulada = onda_lv_1_acumulada + f_onda_lv_1^2;
            check_ortogonal = check_ortogonal + f_onda_lv_2 * f_onda_lv_1;
...
valor_acumulado = valor_acumulado * vol_element * e * cte            % No nos olvidemos de la e!!!
onda_lv_2_acumulada = onda_lv_2_acumulada * vol_element * cte_onda_2^2
onda_lv_1_acumulada = onda_lv_1_acumulada * vol_element * cte_onda_1^2

check_ortogonal = check_ortogonal * vol_element * cte


frecuencia = valor_acumulado / h
w = 2 * pi * frecuencia