...
bispinor = ObtenParteRadialBispinor(r, factor_gamma_wiki, factor_C, bispinor_base);
...
E_Mom = [pt, px, py, pz]
% Vendría a ser la velocidad en unidades naturales (c=1).
mom_norm = [px, py, pz] / pt

dif_energ_eV = (pt - E_reposo) / c_elec
dif_teorica_eV = (E_elec_lv1 - E_reposo) / c_elec
...
% por unidad de onda).
    ...
    px = -1i * h_bar_c * phi_conj * gt * dPhi_dx  / dens_prob - Ax;
    py = -1i * h_bar_c * phi_conj * gt * dPhi_dy  / dens_prob - Ay;
    pz = -1i * h_bar_c * phi_conj * gt * dPhi_dz  / dens_prob - Az;    
...
function bispinor = ObtenParteRadialBispinor(r, factor_gamma_wiki, factor_C, bispinor_base)
    Cr = r * factor_C;  
...
function dPhi_dr = ObtenDerivadaRadialSpinors(r, factor_gamma_wiki, factor_C, bispinor)
    ...
    %dPhi_dr = ((factor_gamma_wiki -2 ) / r - factor_C) * bispinor;
    %dPhi_dr = ((factor_gamma_wiki ) / r - factor_C) * bispinor;
    %dPhi_dr = ((factor_gamma_wiki - 1) / r - factor_C / 2) * bispinor;
    %dPhi_dr = ((factor_gamma_wiki) / r - factor_C / 2) * bispinor;
end
